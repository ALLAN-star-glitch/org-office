  ...
  return (
    <>
      <div
        ...
        {/* Overlay for mobile */}
        <div
          className="absolute inset-0 bg-[#165DFC]/40 lg:hidden"
          onClick={() => setIsMobileSidebarOpen(false)}
        ></div>

        {/* MAIN SIDEBAR */}
        <aside
          className={`absolute lg:relative left-0 top-0 bottom-0 ${
            isCollapsed ? "w-20" : "w-64"
          } bg-gradient-to-b from-[#165DFC] to-[#4EA8FF] text-white shadow-2xl min-h-screen border-r border-[#E5E9F2]/30 transform transition-all duration-300 ease-in-out ${
            isMobileSidebarOpen ? "translate-x-0" : "-translate-x-full"
          } lg:translate-x-0`}
        >
          {/* Header */}
          <div className="sticky top-0 z-20 flex items-center justify-between border-b border-white/20 p-4 bg-gradient-to-b from-[#165DFC] to-[#4EA8FF] h-16">
            {!isCollapsed && (
              <h2 className="text-lg font-semibold tracking-wide">OrgOffice</h2>
            )}
            <button
              onClick={() =>
                isLargeScreen
          ...
          >
            {menuGroups.map((group) => (
              <div key={group.group}>
                {!isCollapsed && (
                  <p className="text-[11px] font-semibold text-white/60 uppercase mb-2 tracking-wider">
                    {group.group}
                  </p>
                )}
